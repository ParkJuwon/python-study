...
def majority_vote(labels):
	"""labels는 가장 까까운 데이터부터 가장 먼 데이터 순서로 정렬되어 있다고 가정"""
...
def knn_classify(k, labeled_points, new_point):
	...
	# labeled_points를 가장 가까운 데이터로부터 가장 먼 데이터 순서로 정렬
	by_distance = sorted(labeled_points, key=lambda point_label: distance(point_label[0], new_point))

	# 가장 가까운 k 데이터 포인트의 레이블을 살펴보고
	k_nearest_labels = [label for _, label in by_distance]

	# 투표를 함
	return majority_vote(k_nearest_labels)


# 각 데이터는 ([경도, 위도], 선호하는 프로그래밍언어) 형태로 구성되어 있
...
def plot_cities():
	# 프로그래밍 언어가 키(key), (경도, 위도) 쌍의 값
	plots = {"Java": ([], []), "Python": ([], []), "R": ([], [])}

	# 각 프로그래밍 언어마다 색깔과 마커를 다르게
	...
	# plot_state_borders(plt)  # 주 경계선을 그려주는 함수가 있다고 하자
