...
from stats import median, stdev

from data_science_from_scratch.chap14 import minimize_stochastic, de_mean
...
def p_value(beta_hat_j, sigma_hat_j):
	if beta_hat_j > 0:
		# 만약 계수가 양수라면
		# 더 큰 값이 발생할 확률에 2를 곱해야 한다
		return 2 * (1 - normal_cdf(beta_hat_j / sigma_hat_j))
	else:
		# 아니라면 더 작은 값이 발생할 확률에 2를 곱해줘야 한
...
# 보통 lambda라고 표현하지만 파이썬에선 이미 사용중인 키워드
def ridge_penalty(beta, alpha):
	return alpha * dot(beta[1:], beta[1:])


def squared_error_ridge(x_i, y_i, beta, alpha):
	"""beta를 사용할 때 오류와 페널티의 합을 추청"""
...
# 	stdev([beta[i] for beta in bootstrap_betas])
# 	for i in range(4)]
#
